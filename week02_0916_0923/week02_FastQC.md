...
2. Additionally, copy the data required for analysis.
    ```
    cd fastqc
    rsync -avh --progress --partial /work/u9482849/2026Biomarkers/{assigned_folder}/ ./
    ```
    or
    ```
    rsync -avh --progress --partial /work/u9482849/2026Biomarkers/{assigned_folder}/ /work/{supercomputer_account}/result/fastqc
    ```
...
> * `-h`:          Human-readable; displays file sizes in KB, MB, GB, etc.
> * `--progress`:  Shows transfer progress, speed, and estimated time remaining
> * `--partial`:   Keeps partially transferred files if the connection is interrupted
...
2. 另外，複製分析所需資料
    ```
    cd fastqc
    rsync -avh --progress --partial /work/u9482849/2026Biomarkers/指定資料夾/ ./
    ```
    或
    ```
    rsync -avh --progress --partial /work/u9482849/2026Biomarkers/指定資料夾/ /work/主機帳號/result/fastqc
    ```
...
> - -v 顯示過程
> - -h 人類可讀的檔案大小
> - --progress 顯示目前傳到多少、速度、剩餘時間
> - --partial 如果中途 SSH 斷線，保留未完成檔案
